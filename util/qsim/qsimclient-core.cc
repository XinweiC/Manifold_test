    ...
    m_Qsim_cpuid(cpuid)
{
    ...
    stringstream ss;
    ss << cpuid;
    trace_file.open(("freqmine_128_"+ ss.str()).c_str());
}
...
fprintf(stdout,"\n[%lld][Core%d]Interrupt seen in md_fetch_next_PC",core->sim_cycle,core->id);
	    ...
	    int rc = core->m_Qsim_client->run(core->m_Qsim_cpuid, 1);
	    if(rc == 0 && core->m_Qsim_client->booted(core->m_Qsim_cpuid) == false) {//no more instructions
...
fprintf(stdout,"\n[%lld][Core%d]Trace DequeuedPC: 0x%llx   ",core->sim_cycle,core->id,q_ptr->front().data.inst.vaddr);
	    ...
	    while (q_ptr->front().type == Qsim::QueueItem::MEM) {
            ...
            trace_file <<"0x"<<hex<<inst->paddr<<" ";
	    trace_file <<dec<< int(inst->qemu_len)<<" "<<inst->mem_ops.memops<<" ";
	    for (unsigned i = 0; i < inst->qemu_len; i++) {
		trace_file<<hex<< int(inst->code[i])<<" "; 
	    }
	    trace_file<<dec<< endl;

	    for (unsigned i = 0; i < 2; i++)
	    {
		if (inst->mem_ops.mem_paddr_ld[i] != 0)
		    trace_file<<"0x0"<<hex<<" 0x"<<inst->mem_ops.mem_paddr_ld[i]<<dec<< " " << int(inst->mem_ops.ld_size[i]) << endl;
		if (inst->mem_ops.mem_paddr_str[i] != 0)
		    trace_file<<"0x1"<<hex<<" 0x"<<inst->mem_ops.mem_paddr_str[i]<<dec<<" " << int(inst->mem_ops.str_size[i]) << endl;
	    }

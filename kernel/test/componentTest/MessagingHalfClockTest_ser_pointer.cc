...
class MyPkt {
	...
	static int Dtor_count; //counter of # of times destructor called.
	~MyPkt() { Dtor_count++; }
};

int MyPkt :: Dtor_count = 0;
...
class MyObj1 : public Component {
	...
	void handler(MyPkt* data)
	{
		...
		m_dbg_log << "    sendHalfTick= " << now
	        ...
	        delete data;
	    //here data is not deleted on LP 0 because we want to send data
	    //to another LP.
	}
...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    Manifold :: ConnectClockHalf(c1, MyObj1::C1_IN, c0, MyObj1::C0_OUT, Clock1, &MyObj1::process_incoming, MyObj1::C0_OUT_LATENCY);

	    MyObj1* comp = (Mytid == 0) ? c0p : c1p;

	    MyPkt* send_data[SIZE];
	    MyPkt send_data_copy[SIZE]; //save a copy for verification
	    // schedule a few events
	    for(int i=0; i<SIZE; i++) {
		send_data[i] = new MyPkt;  //send_data will be deleted by serialization
		send_data[i]->f1 = (char)(random() & 0xffffffff);
		send_data[i]->f2 = (int)(random() & 0xffffffff);
		send_data[i]->f3 = (char)(random() & 0xffffffff);

		send_data_copy[i] = *send_data[i];
		...
		//verify send_data[i] has been deleted by checking the number of times the destructor
		//of MyPkt has been called. Not very robust though.
		CPPUNIT_ASSERT_EQUAL(SIZE, MyPkt::Dtor_count);

		for(int i=0; i<SIZE; i++) {
		    //CPPUNIT_ASSERT_EQUAL(send_data[i], MyPkt :: Deserialize(recv_data[i], sizeof(MyPkt)));
		    CPPUNIT_ASSERT(send_data_copy[i] == Deserialize<MyPkt>(recv_data[i], sizeof(MyPkt)));

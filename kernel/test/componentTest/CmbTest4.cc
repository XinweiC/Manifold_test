//!
//! @brief This program tests the CMB Null-message algorithm.
//!
...
public:
	...
	if(cmbAlg->m_eits.size() > 0) {
	    ...
	    CPPUNIT_ASSERT(Manifold::NowTicks() * m_clock.period <= low);
	}
...
public:
	...
	ofstream DBG_LOG(buf);
        ...
        //all LP must use same seed for random number
...
DBG_LOG << endl;
	...
	//create one extra number as stop time. For this last number, all LPs should have the same value.
        unsigned long last_num[noOfLps];
...
{
    Manifold :: Init(argc, argv, Manifold::TICKED, SyncAlg::SA_CMB, Lookahead::LA_GLOBAL);

...
//! In this program data is passed by pointer, however, the data are static and not allocated
//! from free mem so the handler does not delete them. In this case, we must redefine
//! Serialize/Deseiralize. We override new/delete to verify there's
//! no memory leak.
//!
...
class MyPkt {
	...
	//overriding new and delete so we can make sure there's no mem leak
...
int MyPkt :: New_count = 0;
int MyPkt :: Delete_count = 0;

void* MyPkt :: operator new (size_t size)
{
    New_count++;
    return malloc(size);
}

void MyPkt :: operator delete (void* p)
{
    Delete_count++;
    free(p);
}
...
namespace kernel {
/*
//Specialize Free_serialized_data()
template<>
void Free_serialized_data<MyPkt>(MyPkt*, unsigned char*& buf)
{} //do nothing.
*/
template<>
size_t Serialize<MyPkt>(MyPkt* data, unsigned char* buf)
{
    MyPkt* p = (MyPkt*) buf;
    *p = *data;
    return sizeof(MyPkt);
...
template<>
MyPkt* Deserialize<MyPkt>(unsigned char* buf)
{
    static MyPkt p;
    p = *((MyPkt*)buf);  //byte-wise copy
    return &p;
}
}
}
...
class MyObj1 : public Component {
	...
	void process_incoming(int inputIndex, MyPkt* data)
	{
	    ...
	    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	    //Data are static and being reused, so don't delete here!
	    //delete data; // handler frees memory
	    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	}
};
...
class MessagingTest : public CppUnit::TestFixture {
        ...
        void test_messaging()
	{
	    ...
	    Manifold :: Connect(c1, MyObj1::C1_IN, c0, MyObj1::C0_OUT, &MyObj1::process_incoming, MyObj1::C0_OUT_LATENCY);


	    MyObj1* comp = (Mytid == 0) ? c0p : c1p;


	    //MyPkt send_data[SIZE]; //send_data defined as global
		...
		for(int i=0; i<SIZE; i++) {
		    CPPUNIT_ASSERT(send_data[i] == Deserialize<MyPkt>(recv_data[i], sizeof(MyPkt)));
		...
		for(int i=0; i<SIZE; i++) {
		    //unsigned char* buf;
		    unsigned char buf[sizeof(MyPkt)];
		    Serialize(in_pkt[i], buf);
	    ...
	    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	    //verify no mem leak
	    CPPUNIT_ASSERT_EQUAL(MyPkt::New_count, MyPkt::Delete_count);
	    CPPUNIT_ASSERT_EQUAL(0, MyPkt::New_count);
	}

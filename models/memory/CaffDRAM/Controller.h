...
public:
	...
	Controller(manifold::kernel::Clock *clk, int nid, const Dsettings&, int out_credits, bool st_resp=false);
	~Controller();
...
void Controller :: handle_request(int, uarch::NetworkPacket* pkt)
{
    ...
    if(stats_requests_count > 0) {
        stats_non_zero_requests_ticks += (manifold::kernel::Manifold::NowTicks() - stats_last_requests_count_change_tick);
    }

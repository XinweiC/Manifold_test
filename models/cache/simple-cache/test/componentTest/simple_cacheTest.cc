...
//! The simple_cache is both an L1 and last-level cache.
//!
//!                      --------
//!                     | source |
//!                      --------
...
//! This class plays the role of a memory controller.
class Sink : public manifold::kernel::Component {
public:
    static const int LATENCY = 37;

    Sink() : m_send_reply(false) {}

    void handle_incoming_mem_req(int, Mem_req* req)
    {
    ...
    vector<Mem_req>& get_mem_reqs() { return m_mem_reqs; }
    vector<Ticks_t>& get_ticks() { return m_ticks; }
...
public:
	...
	Manifold::Connect(cacheId, simple_cache::PORT_UPPER, sinkId, 0,
			  &Sink::handle_incoming_mem_req, CACHE_SINK);
	//sink to cache
	Manifold::Connect(sinkId, 0, cacheId, simple_cache::PORT_UPPER,
			  &simple_cache::handle_response, CACHE_SINK);
    }
	...
	CPPUNIT_ASSERT(creq == wrapper->preq);
        ...
        //verify the sink gets a mem_req.
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();
	Mem_req mreq = mreqs[0];
	...
	CPPUNIT_ASSERT_EQUAL(scheduledAt1+SRC_CACHE+HT_HIT, pticks[1]);
        ...
        //verify the sink gets 2 mem_req.
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();
	...
	CPPUNIT_ASSERT_EQUAL(scheduledAt0+SRC_CACHE+HT_LOOKUP+Sink::LATENCY+HT_HIT, pticks[0]);
        ...
        //verify the sink component gets only ONE  mem_req.
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();
	...
	CPPUNIT_ASSERT_EQUAL(1, hits);
        ...
        //verify sink gets 1+(num_of_indices -1) mem_reqs
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();
	...
	CPPUNIT_ASSERT_EQUAL(scheduledAt0+SRC_CACHE+HT_LOOKUP+Sink::LATENCY+HT_HIT, pticks[0]);
        ...
        //verify sink gets 1+num_tags mem_reqs
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();
	...
	CPPUNIT_ASSERT_EQUAL(num_of_indices+1, (int)cresps.size());
        ...
        //verify sink gets num_of_indices +1 mem_reqs
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();
	...
	CPPUNIT_ASSERT_EQUAL(num_tags+1, (int)cresps.size());
        ...
        //verify sink gets num_tags + 1 mem_reqs
        vector<Mem_req>& mreqs = m_sinkp->get_mem_reqs();
        vector<Ticks_t>& ticks = m_sinkp->get_ticks();

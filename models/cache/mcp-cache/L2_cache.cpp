      ...
      DOWNSTREAM_FULL_CREDITS(settings.downstream_credits)
{
    ...
    this->my_table = new hash_table (parameters, 1);
    ...
    this->mshr = new hash_table (mshr_settings, 0);
...
{
    DBG_L2_CACHE_TICK_ID(cerr, "######  handle_incoming()_coh, srcID= " << request->src_id << " type= " << int(request->type) << " addr=(" <<hex<< request->addr <<dec<< ")" << endl);
...
{
    DBG_L2_CACHE_TICK_ID(cerr,  "######## process_mem_resp(), addr= " << hex << request->addr << dec << endl);
...
{
    DBG_L2_CACHE_ID(cerr, "process_client_request(), addr= " << hex << request->addr << dec << endl);
	...
	DBG_L2_CACHE_ID(cerr, "    PREV_PEND_STALL, addr= " << hex << request->addr << dec << endl);
    ...
    if (mshr_entry == 0)
    {
	DBG_L2_CACHE_ID(cerr, "  MSHR_STALL, addr= " << hex << request->addr << dec << endl);
    ...
    {
	DBG_L2_CACHE(cerr, "    L2_cache: request is miss.\n");
	...
	if(managers[0]->is_invalidation_request(request)) {
	    DBG_L2_CACHE(cerr, "    L2_cache: a missed invalidation request; ignore it.\n");
	...
	hash_table_entry = my_table->reserve_block_for (request->addr);
	    ...
	    DBG_L2_CACHE(cerr, "    L2_cache start replacement, victim line addr= " <<hex<< victim->get_line_addr() <<dec<< "\n");
		...
		DBG_L2_CACHE(cerr, "    LRU BUSY_STALL waiting for " <<hex<< my_table->get_replacement_entry(request->addr)->get_line_addr() <<dec<< "\n");
    ...
    {
	DBG_L2_CACHE(cerr, "    L2_cache :: process_client_request(), request hits in hash table.\n");
	...
	hash_table_entry = my_table->get_entry(request->addr);
	    ...
	    DBG_L2_CACHE(cerr, "    L2_cache: TRANS_STALL\n");
...
{
    DBG_L2_CACHE(cerr, "    L2_cache :: L2_algorithm(), addr= " << hex << request->addr << dec << endl);
...
{
    DBG_L2_CACHE(cerr, "    start_eviction().\n");
...
{
    DBG_L2_CACHE_ID(cerr,  "process_client_reply(), addr= " << hex << reply->addr << dec << endl);

    hash_entry* mshr_entry = mshr->get_entry(reply->addr);

    if(mshr_entry) { //this is a response to a request

	DBG_L2_CACHE(cerr,  "    mshr found\n");
...
{
    DBG_L2_CACHE(cerr,  "    m_evict_notify\n");

    Coh_msg* req = mcp_stalled_req[manager->getManagerID()];
    assert(req);
    DBG_L2_CACHE(cerr, "    There is a request waiting for eviction to finish, req= " << req << " src= " << req->src_id << " msg= " << req->msg << " addr= " <<hex<< req->addr <<dec<< "\n");
...
void L2_cache::get_from_memory (Coh_msg *request)
{
    ...
    *((Mem_msg*)(pkt->data)) = req;
    ...
    DBG_L2_CACHE_ID(cerr,  " get from memory node " << req.dst_id << " for 0x" << hex << req.addr << dec << endl);
...
{
    DBG_L2_CACHE_ID(cerr,  " dirty write to memory for 0x" << hex << addr << dec << endl);
...
void L2_cache :: send_msg_to_l1(Coh_msg* msg)
{
    ...
    *((Coh_msg*)(pkt->data)) = *msg;
    ...
    DBG_L2_CACHE_ID(cerr,  " sending msg= " << msg->msg << " to L1 node= " << msg->dst_id << " fwd= " << msg->forward_id << endl);
...
{
    DBG_L2_CACHE_ID(cerr, " sending credit.\n");
...
{
    DBG_L2_CACHE_ID(cerr, " client write back.\n");
...
{
    DBG_L2_CACHE_ID(cerr,  " invalidating " << hash_entries[mgr->getManagerID()] << " id= " << hash_entries[mgr->getManagerID()]->get_idx() << endl);
...
{
    DBG_L2_CACHE_ID(cerr,  " ignore a request.\n");
}
...
void L2_cache :: release_mshr_entry(hash_entry* mshr_entry)
{
	...
	DBG_L2_CACHE_ID(cerr,  " wakeup req, stall type= " << stallType << " req= " << req << " msg= " << req->msg << " addr= " <<hex<< req->addr <<dec << " src= " << req->src_id << "\n");

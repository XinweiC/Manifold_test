...
public:
    enum {CREDIT_TYPE = 12345};

    MockMux(MESI_LLP_cache* c) : m_llp_cache(c), MuxDemux(Clock::Master(), CREDIT_TYPE)  {}
...
public:
    ...
    {
	if(pkt->type == MockMux :: CREDIT_TYPE)
...
public:
    ...
    {
        if(((addr >> 12) & 0x3) == 0) //page number ending with 00
	    return NODE_ID;
	else
	    return OTHER_NODE_ID;
    }

    //create a random address that maps to the local L2.
    paddr_t generate_local_addr()
    {
        paddr_t addr = random();
	paddr_t mask = (0x3 << 12);
	mask = ~mask;
    ...
    //create a random address that maps to the remote L2.
    paddr_t generate_remote_addr()
    {
        paddr_t addr = random();
	paddr_t mask = (0x3 << 12);
	mask = ~mask;
	addr &= mask; //clear page number LSBs
	mask = random() % 3 + 1; //randomly select 1,2,3
	mask <<= 12;
	return addr | mask;
    }
};
...
public:
	...
	CompId_t cacheId = Component :: Create<MESI_LLP_cache>(0, NODE_ID, cache_parameters, settings);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): load miss; remote L2
    //!
    //! Empty cache; load with a random address; since this is a miss, client
    //! sends MESI_CM_I_TO_S.
    //! Since the msg is sent to a remote L2, downstream_credits is decremented.
	...
	paddr_t ADDR = m_l2map->generate_remote_addr();
	CPPUNIT_ASSERT(m_l2map->lookup(ADDR) != NODE_ID);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): load miss; local L2
    //!
    //! Empty cache; load with a random address; since this is a miss, client
    //! sends MESI_CM_I_TO_S.
    //! Since the msg is sent to local L2, downstream_credits is NOT decremented.
    void test_handle_processor_request_load_l1_I_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits
	mySetUp(CREDITS);
	...
	//create a LOAD request
	paddr_t ADDR = m_l2map->generate_local_addr();
	CPPUNIT_ASSERT(m_l2map->lookup(ADDR) == NODE_ID);
	...
	Manifold::Run();
        ...
        //verify the downstream credits is NOT decremented
	CPPUNIT_ASSERT_EQUAL(CREDITS, m_cachep->m_downstream_credits);
    }
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): load hit; remote L2
    //!
    ...
    //! Since no msg is sent, downstream_credits is unchanged.
	...
	//create a LOAD request
	const paddr_t ADDR = m_l2map->generate_remote_addr();
	CPPUNIT_ASSERT(m_l2map->lookup(ADDR) != NODE_ID);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): load hit; local L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    //! the corresponding client to E, S, or M. Send a request with the same
    //! address; it should hit, and a response is sent back.
    //! Since no msg is sent, downstream_credits is unchanged.
    void test_handle_processor_request_load_l1_ESM_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits
	mySetUp(CREDITS);

	//create a LOAD request
	const paddr_t ADDR = m_l2map->generate_local_addr();
	CPPUNIT_ASSERT(m_l2map->lookup(ADDR) == NODE_ID);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store miss; remote L2
    //!
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store miss; local L2
    //!
    //! Empty cache; store with a random address; since this is a miss, client
    //! sends a MESI_CM_I_TO_E message.
    //! Since the msg is sent to a local L2, downstream_credits is NOT decremented.
    void test_handle_processor_request_store_l1_I_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits
	mySetUp(CREDITS);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store hit in M state; remote L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store hit in M state; local L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    //! the corresponding client to M. Send a request with the same
    //! address; it should hit, and a response is sent back.
    //! Since no msg is sent, downstream_credits is unchanged.
    void test_handle_processor_request_store_l1_M_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits
	mySetUp(CREDITS);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store hit in E state; remote L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store hit in E state; local L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    //! the corresponding client to E. Send a request with the same
    //! address; it should hit, and a response is sent back. Client state changes to M.
    //! Since no msg is sent, downstream_credits is unchanged.
    void test_handle_processor_request_store_l1_E_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits
	mySetUp(CREDITS);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store hit in S state; remote L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): store hit in S state; local L2
    //!
    //! Empty cache; manually put a tag in the hash table and set the state of
    //! the corresponding client to S. Send a request with the same
    //! address; it should hit; a MESI_CM_I_to_E request is sent to manager;
    //! client enters SE.
    //! Since the msg is sent to local L2, downstream_credits is NOT decremented.
    void test_handle_processor_request_store_l1_S_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits
	mySetUp(CREDITS);
...
//########
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in E; owner in E; remote L2
    //!
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! Same as above; only initialize credits to 1; only one message can be sent.
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
	...
	//verify downstream gets only 1 packet
	CPPUNIT_ASSERT_EQUAL(1, (int)m_muxp->get_pkts().size());

	//verify downstream gets 1 credit
	CPPUNIT_ASSERT_EQUAL(1, (int)m_netp->get_credit_pkts().size());
    ...
    //======================================================================
    //======================================================================
    //! Same as above; initialize credits to 1; send a credit from downstream to cache.
    //! 2 msgs are sent out.
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
	...
	Manifold::Schedule(When, &MockMux::send_pkt, m_muxp, credit);
	When += PROC_CACHE + HT_LOOKUP + 10;
	...
	//verify downstream gets 2 packets
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in E; owner in E; local L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
    //! 1. Client --> manager, MESI_CM_I_to_S; client enters IE state.
    //! 2. Manager(in E) --> owner, MC_FWD_S.
    ...
    //!
    //! Since the msg is sent to local L2, downstream_credits is not decremented.
    void test_handle_peer_and_manager_request_load_l1_I_l2_E_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
	...
	//verify local l2 gets 2 Coh_msg; mux gets no packets
	CPPUNIT_ASSERT_EQUAL(2, (int)m_l2p->get_mem_reqs().size());
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in E; owner in M; remote L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! same as above; credits initialized to 1. Only one message can be sent out.
    //!
    void test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
	...
	//verify downstream gets 1 packet
	CPPUNIT_ASSERT_EQUAL(1, (int)m_muxp->get_pkts().size());
    ...
    //======================================================================
    //======================================================================
    //! same as above; credits initialized to 1. Send a credit to cache. 2 msgs
    //! are sent out by cache.
    //!
    void test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in E; owner in M; local L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
    //! 1. Client --> manager, CM_I_to_S; client enters IE state.
    //! 2. Manager(in E) --> owner, MC_FWD_S.
    ...
    //! 4. Client --> manager, MESI_CM_UNBLOCK_S; client enters S state.
    //! 4. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in S; remote L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
    //! 1. Client --> manager, CM_I_to_S; client enters IE state.
	...
	*((Coh_msg*)(mgrPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! Same as above. Initialize credits to 1. Only one msg gets sent.
    //!
    void test_handle_peer_and_manager_request_load_l1_I_l2_S_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! Same as above. Initialize credits to 1. Send a credit to cache. 2 msgs
    //! are sent out.
    //!
    void test_handle_peer_and_manager_request_load_l1_I_l2_S_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in S; local L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
    //! 1. Client --> manager, CM_I_to_S; client enters IE state.
    //! 2. Manager(in S) --> client, MC_GRANT_S_DATA.
    //! 3. Client --> manager, CM_UNBLOCK_S ; client enters S state.
    //! 4. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_load_l1_I_l2_S_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = req;
	...
	//verify locla l2 gets 2 Coh_msg; mux gets no packets.
	CPPUNIT_ASSERT_EQUAL(2, (int)m_l2p->get_mem_reqs().size());
	CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_pkts().size());
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in I; remote L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
    //! 1. Client --> manager, CM_I_to_S; client enters IE state.
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //======================================================================
    //! Same as above; Initialize credits to 1; Only one msg is sent by cache.
    void test_handle_peer_and_manager_request_load_l1_I_l2_I_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //======================================================================
    //! Same as above; Initialize credits to 1; Send a credit to cache. Two
    //! msgs are sent by cache.
    void test_handle_peer_and_manager_request_load_l1_I_l2_I_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): load miss; manager in I; local L2
    //!
    //! Empty cache; load with a random address; the following sequence happens:
    //! 1. Client --> manager, CM_I_to_S; client enters IE state.
    //! 2. Manager --> client,  MC_GRANT_E_DATA; manager enters IE state.
    //! 3. Client --> manager, CM_UNBLOCK_E; client enters E state.
    //! 4. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_load_l1_I_l2_I_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	//verify downstream gets 0 credit, since reply to LLP is from local L2.
	CPPUNIT_ASSERT_EQUAL(0, (int)m_netp->get_credit_pkts().size());
    }
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in E; owner in E; remote L2
    //!
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_E_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_E_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in E; owner in E; local L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
    //! 1. Client --> manager, CM_I_to_E; client enters IE state.
    //! 2. Manager(in E) --> owner,  MC_FWD_E.
    ...
    //! 4. Client --> manager, CM_UNBLOCK_E to manager; client enters E state, then to M state.
    //! 5. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_store_l1_I_l2_E_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in E; owner in M; remote L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in E; owner in M; local L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
    //! 1. Client sends MESI_CM_I_to_E to manager; client enters IE state.
    //! 2. Manager in E state, so it sends MESI_MC_FWD_E to owner, which in turn sends
    ...
    //! 3. Client sends MESI_CM_UNBLOCK_E to manager; client enters M state.
    //! 4. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(ownerPkt->data)) = req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in S; remote L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
    //! 1. Client sends MESI_CM_I_to_E to manager; client enters IE state.
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_S_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_S_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in S; local L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
    //! 1. Client sends MESI_CM_I_to_E to manager; client enters IE state.
    //! 2. Manager in S state, so it invalidates all shares, then sends
    ...
    //! 3. Client sends MESI_CM_UNBLOCK_E to manager; client enters E state, then M state.
    //! 4. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_store_l1_I_l2_S_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	//verify downstream gets no credits since reply was sent by local L2.
	CPPUNIT_ASSERT_EQUAL(0, (int)m_netp->get_credit_pkts().size());
    }
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in I; remote L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
    //! 1. Client sends MESI_CM_I_to_E to manager; client enters IE state.
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_I_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_store_l1_I_l2_I_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): store miss; manager in I; local L2
    //!
    //! Empty cache; store with a random address; the following sequence happens:
    //! 1. Client sends MESI_CM_I_to_E to manager; client enters IE state.
    //! 2. Manager in I state, so it sends MESI_MC_GRANT_E_DATA to client; manager enters IE state.
    //! 3. Client sends MESI_CM_UNBLOCK_E to manager; client enters E state, then to M state.
    //! 4. L1 cache now sends a response back to processor.
    void test_handle_peer_and_manager_request_store_l1_I_l2_I_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	//verify downstream gets no credits since reply was sent by local L2.
	CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_credit_pkts().size());
    }
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req;
    ...
    //! 2. Manager --> client, MC_GRANT_E_DATA.
    //!
    void test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
...
//#####################
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_E; owner in E; remote L2
    //!
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_E; owner in E; local L2
    //!
    //! Test a owner's behavior when it's in E state and receives MESI_MC_FWD_E: it sends
    //! MESI_CC_E_DATA and invalidates itself.
    //! 1. put ADDR in hash table and client in E state.
    //! 2. C<---M, MESI_MC_FWD_E;
    //! 4. C--->forwardee, MESI_CC_E_DATA; client invalidated.
    void test_handle_peer_and_manager_request_owner_E_recv_FWD_E_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits; at least 1
	mySetUp(CREDITS);
	...
	Manifold::Run();
        ...
        //verify the downstream credits is decremented since msg is sent to another L1
	CPPUNIT_ASSERT_EQUAL(CREDITS - 1, m_cachep->m_downstream_credits);

	//verify local L2 gets no Coh_msg; mux gets 1 packet.
	CPPUNIT_ASSERT_EQUAL(0, (int)m_l2p->get_mem_reqs().size());
	CPPUNIT_ASSERT_EQUAL(1, (int)m_muxp->get_pkts().size());
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_E; owner in M; remote L2
    //!
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_E; owner in M; local L2
    //!
    //! Test a owner's behavior when it's in M state and receives MESI_MC_FWD_E: it sends
    //! MESI_CC_M_DATA and invalidates itself.
    //! 1. put ADDR in hash table and client in M state.
    ...
    //! 3. C--->forwardee, MESI_CC_M_DATA; client invalidated.
    //!
    void test_handle_peer_and_manager_request_owner_M_recv_FWD_E_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits; at least 1
	mySetUp(CREDITS);
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_S; owner in E; remote L2
    //!
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_owner_E_recv_FWD_S_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_owner_E_recv_FWD_S_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_S; owner in E; local L2
    //!
    //! Test a owner's behavior when it's in E state and receives MESI_MC_FWD_S: it sends
    //! MESI_CC_S_DATA and enters S state.
    //! 1. put ADDR in hash table and client in E state.
    ...
    //! 3. C--->forwardee, MESI_CC_S_DATA
    //! 4. C--->Manager, MESI_CM_CLEAN; client enters S.
    void test_handle_peer_and_manager_request_owner_E_recv_FWD_S_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
	...
	//verify local L2 gets 1 Coh_msg; mux gets 1 packet.
	CPPUNIT_ASSERT_EQUAL(1, (int)m_l2p->get_mem_reqs().size());
	CPPUNIT_ASSERT_EQUAL(1, (int)m_muxp->get_pkts().size());
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_S; owner in M; remote L2
    //!
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //======================================================================
    //! Same as above; Initialize credits to 1; Only 1 msg is sent by cache.
    void test_handle_peer_and_manager_request_owner_M_recv_FWD_S_1()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //! Same as above; Initialize credits to 1; Send a credit to cache; 2 msgs
    //! are sent by cache.
    void test_handle_peer_and_manager_request_owner_M_recv_FWD_S_2()
    {
	int CREDITS = 1;
	mySetUp(CREDITS);
	...
	*((Coh_msg*)(mgrPkt->data)) = *req2;
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_peer_and_manager_request(): receive MESI_MC_FWD_S; owner in M; local L2
    //!
    //! Test a owner's behavior when it's in M state and receives MESI_MC_FWD_S: it sends
    //! MESI_CC_S_DATA and MESI_CM_WRITEBACK, and enters S state.
    //! 1. put ADDR in hash table and client in M state.
    ...
    //! 4. C--->M, MESI_CM_WRITEBACK
    //!
    void test_handle_peer_and_manager_request_owner_M_recv_FWD_S_3()
    {
	int CREDITS = random() % 100 + 2; //downstream credits; at least 2
	mySetUp(CREDITS);
...
//########
	...
	Manifold::Run();
	    ...
	    //verify local L2 gets 1 Coh_msg; mux gets nothing.
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_pkts().size());
	}
	else {
	...
	//verify downstream gets 0 credit
	CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_credit_pkts().size());
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): load miss; evicted in S; remote L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
	...
	Manifold::Run();
        ...
        //After the silent eviction; a CM_I_to_S is sent to manager.
    ...
    //======================================================================
    //======================================================================
    //! @brief Test handle_processor_request(): load miss; evicted in S; local L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
    //! Since the evicted entry is in S, it transits to I state; no messages to the manager.
    void test_handle_processor_request_load_eviction_l1_S_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits; at least 1
	mySetUp(CREDITS);
    ...
    //! load miss; evicted in E; remote L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
	...
	//There are 3 messages sent out: CM_E_to_I and CM_UNBLOCK_I for VICTIM_ADDR,
	//and CM_I_to_S for ADDR
        if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID) {
	    //verify the downstream credits is decremented
	    CPPUNIT_ASSERT_EQUAL(CREDITS - 1, m_cachep->m_downstream_credits);

	    //verify local L2 gets 2 Coh_msg; mux gets 1.
	    CPPUNIT_ASSERT_EQUAL(2, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_muxp->get_pkts().size());

	    //verify downstream gets 0 credit for MC_GRANT_I
	    CPPUNIT_ASSERT_EQUAL(0, (int)m_netp->get_credit_pkts().size());
	}
	else {
	    //verify the downstream credits is decremented
	    CPPUNIT_ASSERT_EQUAL(CREDITS - 3, m_cachep->m_downstream_credits);

	    //verify downstream gets 3 packets
	    CPPUNIT_ASSERT_EQUAL(3, (int)m_muxp->get_pkts().size());

	    //verify downstream gets 1 credit for MC_GRANT_I
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_netp->get_credit_pkts().size());
	}
    ...
    //======================================================================
    //! @brief Test handle_processor_request() and handle_peer_and_manager_request();
    //! load miss; evicted in E; local L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
    //! 1. victim--->M, MESI_CM_E_to_I.
    ...
    //! Now the new request is woken up.
    //! 1. C--->M, MESI_CM_I_to_S.
    void test_handle_peer_and_manager_request_load_eviction_l1_E_1()
    {
	int CREDITS = random() % 100 + 3; //downstream credits; at least 3
	mySetUp(CREDITS);
	...
	//and CM_I_to_S for ADDR
	    ...
	    //verify local L2 gets 3 Coh_msg; mux gets 0.
	    CPPUNIT_ASSERT_EQUAL(3, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_pkts().size());
	    ...
	    //verify local L2 gets 1 Coh_msg; mux gets 2.
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(2, (int)m_muxp->get_pkts().size());
    ...
    //======================================================================
    //! @brief Test handle_processor_request() and handle_peer_and_manager_request():
    //! load miss; evicted in S; remote L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
    //!
    ...
    //======================================================================
    //! @brief Test handle_processor_request() and handle_peer_and_manager_request():
    //! load miss; evicted in S; local L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
    //!
    ...
    //! continues:
    //! 1. C--->M, MESI_CM_I_to_S.
    void test_handle_peer_and_manager_request_load_eviction_l1_S_1()
    {
	int CREDITS = random() % 100 + 1; //downstream credits; at least 1
	mySetUp(CREDITS);
	...
	//verify local L2 gets 1 Coh_msg; mux gets 0.
	CPPUNIT_ASSERT_EQUAL(1, (int)m_l2p->get_mem_reqs().size());
	CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_pkts().size());
    ...
    //======================================================================
    //! @brief Test handle_processor_request() and handle_peer_and_manager_request():
    //! load miss; evicted in M; remote L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
	...
	//There are 3 messages sent out: CM_M_to_I and CM_UNBLOCK_I for VICTIM_ADDR,
	//and CM_I_to_S for ADDR
        if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID) {
	    //verify the downstream credits is decremented
	    CPPUNIT_ASSERT_EQUAL(CREDITS - 1, m_cachep->m_downstream_credits);
    ...
    //======================================================================
    //! @brief Test handle_processor_request() and handle_peer_and_manager_request():
    //! load miss; evicted in M; local L2
    //!
    //! Fill up a cache set; then LOAD with a new tag for the same set. This
    //! causes the LRU entry to be evicted.
    //! 1. victim--->M, MESI_CM_M_to_I.
    ...
    //! Now the new request is woken up.
    //! 1. C--->M, MESI_CM_I_to_S.
    void test_handle_peer_and_manager_request_load_eviction_l1_M_1()
    {
	int CREDITS = random() % 100 + 3; //downstream credits; at least 3
	mySetUp(CREDITS);
...
//############
	...
	if(m_l2map->lookup(ADDR) == NODE_ID) {
	    ...
	    //verify local L2 gets 1 Coh_msg; mux gets 0.
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_l2p->get_mem_reqs().size()); //CM_UNBLOCK_I
	    CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_pkts().size());
	    ...
	    //verify local L2 gets 0 Coh_msg; mux gets 1.
	    CPPUNIT_ASSERT_EQUAL(0, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_muxp->get_pkts().size());
	...
	if(m_l2map->lookup(ADDR) == NODE_ID) {
	    ...
	    //verify local L2 gets 1 Coh_msg; mux gets 0.
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_l2p->get_mem_reqs().size()); //CM_UNBLOCK_I_DIRTY
	    CPPUNIT_ASSERT_EQUAL(0, (int)m_muxp->get_pkts().size());
...
//##########
	...
	int n_local=0;
	for(int i=0; i<NUM_TAGS; i++) {
	    if(m_l2map->lookup(addrs[i]) == NODE_ID)
	        n_local++;
	}

	    //verify the downstream credits is decremented
	    CPPUNIT_ASSERT_EQUAL(CREDITS - (NUM_TAGS-n_local), m_cachep->m_downstream_credits);

	    //verify local L2 and mux
	    CPPUNIT_ASSERT_EQUAL(n_local, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(NUM_TAGS - n_local, (int)m_muxp->get_pkts().size());
	...
	if(m_l2map->lookup(ADDR) == NODE_ID) {
	    ...
	    //verify the downstream credits is decremented
	    //Total messages sent to L2 is NUM_TAGS+2: NUM_TAGS CM_I_to_S, CM_UNBLOCK_S, and CM_I_to_S
	    CPPUNIT_ASSERT_EQUAL(CREDITS - (NUM_TAGS+2-n_local), m_cachep->m_downstream_credits);

	    //verify local L2 and mux
	    CPPUNIT_ASSERT_EQUAL(n_local, (int)m_l2p->get_mem_reqs().size());
	    CPPUNIT_ASSERT_EQUAL(NUM_TAGS + 2 - n_local, (int)m_muxp->get_pkts().size());
...
cout << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n";
	...
	//MESI_CM_I_to_S, MESI_CM_UNBLOCK_E, both for 1st request
	...
	//MESI_CM_I_to_S, MESI_CM_UNBLOCK_E, both for 1st request
	//2nd request store hit, so it changes to M without sending messages.
...
//##############
	...
	//Total messages sent to L2 is NUM_TAGS + 2
	int n_local=0;
	for(int i=0; i<NUM_TAGS; i++) {
	    if(m_l2map->lookup(addrs[i]) == NODE_ID)
	        n_local++;
...
//###########
	...
	//Only one msg sent to L2: CM_E_to_I
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID) {
	    CPPUNIT_ASSERT_EQUAL(CREDITS, m_cachep->m_downstream_credits);
	...
	//5 msg sent to L2: CM_E_to_I, CM_UNBLOCK_I, CM_I_to_S(ADDR), CM_UNBLOCK_E, CM_E_to_I(ADDR2 causes another eviction)
	int n_local = 0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID) {
	    n_local += 2;
	}
	...
	if(m_l2map->lookup(addrs[1]) == NODE_ID) {
	    n_local++;
	}

	CPPUNIT_ASSERT_EQUAL(CREDITS - (5-n_local), m_cachep->m_downstream_credits);
	CPPUNIT_ASSERT_EQUAL(n_local, (int)m_l2p->get_mem_reqs().size());
	CPPUNIT_ASSERT_EQUAL(5-n_local, (int)m_muxp->get_pkts().size());


        int n_remote_reply=0;
	if(m_l2map->lookup(VICTIM_ADDR) != NODE_ID)
	    n_remote_reply++;
	if(m_l2map->lookup(ADDR) != NODE_ID)
	    n_remote_reply++;
	CPPUNIT_ASSERT_EQUAL(n_remote_reply, (int)m_netp->get_credit_pkts().size());
...
//####################
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//2 msgs are sent: CM_E_to_I for VICTIM_ADDR and UNBLOCK_I for ADDR
	int n_local=0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID)
	    n_local++;
	if(m_l2map->lookup(ADDR) == NODE_ID)
...
//##################
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//3 msgs are sent: CM_E_to_I for VICTIM_ADDR, CC_E_DATA for VICTIM_ADDR and CM_I_to_S for ADDR
	//CC_E_DATA is sent to peer
	int n_local=0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID)
	    n_local++;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//3 msgs are sent: CM_E_to_I for VICTIM_ADDR, CC_M_DATA for VICTIM_ADDR and CM_I_to_S for ADDR
	//CC_M_DATA is sent to peer
	int n_local=0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID)
	    n_local++;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//3 msgs are sent: CM_M_to_I for VICTIM_ADDR, CC_M_DATA for VICTIM_ADDR and CM_I_to_S for ADDR
	//CC_M_DATA is sent to peer
	int n_local=0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID)
	    n_local++;
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//3 msgs are sent: CM_E_to_I and CM_UNBLOCK_I for VICTIM_ADDR, and CM_I_to_S for ADDR
	int n_local=0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID)
	    n_local += 2;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//3 msgs are sent: CM_E_to_I and CM_UNBLOCK_I_DIRTY for VICTIM_ADDR, and CM_I_to_S for ADDR
	int n_local=0;
	if(m_l2map->lookup(VICTIM_ADDR) == NODE_ID)
	    n_local += 2;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//2 msgs are sent: CM_I_to_E and CM_UNBLOCK_I for ADDR
	int n_local=0;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	    n_local += 2;
	CPPUNIT_ASSERT_EQUAL(CREDITS - (2-n_local), m_cachep->m_downstream_credits);
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//3 msgs are sent: CM_I_to_E, CM_UNBLOCK_I, CM_UNBLOCK_E for ADDR
	int n_local=0;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	    n_local += 3;
	CPPUNIT_ASSERT_EQUAL(CREDITS - (3-n_local), m_cachep->m_downstream_credits);
	...
	CPPUNIT_ASSERT_EQUAL(3-n_local, (int)m_muxp->get_pkts().size());
        ...
        //2 msg received: MC_DEMAND_I, CC_M_DATA
	if(m_l2map->lookup(ADDR) == NODE_ID)
	    CPPUNIT_ASSERT_EQUAL(1, (int)m_netp->get_credit_pkts().size());
	else
	    CPPUNIT_ASSERT_EQUAL(2, (int)m_netp->get_credit_pkts().size());
	...
	Manifold::Run();
        ...
        //verify the downstream component
	//2 msgs are sent: CM_I_to_E, CM_UNBLOCK_I for ADDR
	int n_local=0;
	if(m_l2map->lookup(ADDR) == NODE_ID)
	    n_local += 2;
	CPPUNIT_ASSERT_EQUAL(CREDITS - (2-n_local), m_cachep->m_downstream_credits);
	...
	*/
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_load_l1_I_1", &MESI_LLP_cacheTest::test_handle_processor_request_load_l1_I_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_load_l1_ESM_1", &MESI_LLP_cacheTest::test_handle_processor_request_load_l1_ESM_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_store_l1_I_1", &MESI_LLP_cacheTest::test_handle_processor_request_store_l1_I_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_store_l1_M_1", &MESI_LLP_cacheTest::test_handle_processor_request_store_l1_M_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_store_l1_E_1", &MESI_LLP_cacheTest::test_handle_processor_request_store_l1_E_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_store_l1_S_1", &MESI_LLP_cacheTest::test_handle_processor_request_store_l1_S_1));
        //response
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_E_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_E_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_E_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_E_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_E_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_E_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_E_owner_M_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_S_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_S_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_S_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_S_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_S_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_S_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_I_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_I_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_I_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_I_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_l1_I_l2_I_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_l1_I_l2_I_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_E_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_E_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_E_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_E_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_E_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_E_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_E_owner_M_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_S_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_S_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_S_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_S_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_S_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_S_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_I_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_I_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_I_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_I_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_l1_I_l2_I_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_l1_I_l2_I_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_store_hit_l1_S_l2_S_3));
	//owner behavior
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_E_recv_FWD_E_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_E_recv_FWD_E_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_M_recv_FWD_E_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_M_recv_FWD_E_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_E_recv_FWD_S_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_E_recv_FWD_S_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_E_recv_FWD_S_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_E_recv_FWD_S_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_E_recv_FWD_S_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_E_recv_FWD_S_3));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_M_recv_FWD_S_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_M_recv_FWD_S_1));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_M_recv_FWD_S_2", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_M_recv_FWD_S_2));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_owner_M_recv_FWD_S_3", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_owner_M_recv_FWD_S_3));
	//eviction
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_processor_request_load_eviction_l1_S_1", &MESI_LLP_cacheTest::test_handle_processor_request_load_eviction_l1_S_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_eviction_l1_E_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_eviction_l1_E_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_eviction_l1_S_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_eviction_l1_S_1));
	...
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_cacheTest>("test_handle_peer_and_manager_request_load_eviction_l1_M_1", &MESI_LLP_cacheTest::test_handle_peer_and_manager_request_load_eviction_l1_M_1));
	//invalidation

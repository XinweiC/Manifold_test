...
//! This program tests MESI_LLP_cache and MESI_LLS_cache as a component. All other
//! elements are mock objects.
//!
//! Note that each MESI_LLP, MESI_LLS, and mux form a LP_LLS_unit
//!
//!       -----------        -----------         -----------
...
//!            |                  |                   |
//!        ----------         ----------          ----------
//!       | MESI_LLP |       | MESI_LLP |        | MESI_LLP |
//!        ----------         ----------          ----------
//!         |      |           |      |            |      |
//!   ----------   |     ----------   |      ----------   |
//!  | MESI_LLS |  |    | MESI_LLS |  |     | MESI_LLS |  |
//!   ----------   |     ----------   |      ----------   |
//!        |-----+ |          |-----+ |           |-----+ |
//!              | |                | |                 | |
//!             -----              -----               -----
...
static const int NODEID_NODE_1 = 2; //a node includes processor, LLP, LLS, mux
static const int NODEID_NODE_2 = 3;
static const int NODEID_NODE_3 = 4;
...
//! This class emulates a processor
...
//! This mapping maps an address to an LLS's node ID based on page.
class MyLLSMap : public DestMap {
public:
    MyLLSMap(vector<int>& nodes, int page_offset_bits) : m_page_offset_bits(page_offset_bits)
    {
	assert(nodes.size() > 0);
    ...
    {
        return m_nodes[(m_selector_mask & (addr >> m_page_offset_bits)) % m_nodes.size()];
    }
...
public:
    ...
    void handle_incoming(int port, NetworkPacket* pkt)
    {
	    ...
	    //for recv credits, we can't tell it's from LLP or LLS.
	    m_recv_credits[port]++;
	    return;
	}
//cout << "Network: " << pkt->src << " --> " << pkt->dst << endl;
	...
	credit->dst = pkt->src;
	credit->dst_port = pkt->src_port;
	Send(port, credit);
	//for send credit, we know which dst_port to send, so it's on dst_port basis
	//do not confuse the dst_port with the network's I/O ports
	//instead of using a pair as the key, we simply shift the port(node) id, so the
	//key is: (port(node) << 8) | dst_port
	m_send_credits[(port << 8) | credit->dst_port]++;
...
//####################################################################
//####################################################################
class MESI_LLP_LLS_cacheTest : public CppUnit::TestFixture {
private:
    static const unsigned LLP_MSHR_SIZE = 8;
    static const unsigned LLS_MSHR_SIZE = 8;

    MockProc* m_procp1;
    MockProc* m_procp2;
    MockProc* m_procp3;
    MESI_LLP_cache* m_l1p1;
    MESI_LLP_cache* m_l1p2;
    MESI_LLP_cache* m_l1p3;
    MESI_LLS_cache* m_l2p1;
    MESI_LLS_cache* m_l2p2;
    MESI_LLS_cache* m_l2p3;
    //MuxDemux* m_muxp1;
    //MuxDemux* m_muxp2;
    //MuxDemux* m_muxp3;
    MockMc* m_mcp1;
...
public:
	...
	vector<int> l2_nodes;
	l2_nodes.push_back(NODEID_NODE_1);
	l2_nodes.push_back(NODEID_NODE_2);
	l2_nodes.push_back(NODEID_NODE_3);
	m_l2map = new MyLLSMap(l2_nodes, PAGE_BITS); //maps an address to an L2 node ID.
	...
	l1_settings.mshr_sz = LLP_MSHR_SIZE;
	l1_settings.downstream_credits = 10;

	L2_cache_settings l2_settings_1, l2_settings_2;
	l2_settings_1.mc_map = mcmap1;
	l2_settings_1.mshr_sz = LLS_MSHR_SIZE;
	l2_settings_1.downstream_credits = 10;

	l2_settings_2.mc_map = mcmap2;
	l2_settings_2.mshr_sz = LLS_MSHR_SIZE;
	l2_settings_2.downstream_credits = 10;


	//create the LP_LLS_units
        LP_LLS_unit* unit1 = new LP_LLS_unit(0, NODEID_NODE_1, l1_parameters, l2_parameters, l1_settings, l2_settings_1, MasterClock, CREDIT_TYPE);
        LP_LLS_unit* unit2 = new LP_LLS_unit(0, NODEID_NODE_2, l1_parameters, l2_parameters, l1_settings, l2_settings_2, MasterClock, CREDIT_TYPE);
        LP_LLS_unit* unit3 = new LP_LLS_unit(0, NODEID_NODE_3, l1_parameters, l2_parameters, l1_settings, l2_settings_1, MasterClock, CREDIT_TYPE);

	m_l1Id1 = unit1->get_llp_cid();
	m_l1p1 = dynamic_cast<MESI_LLP_cache*>(unit1->get_llp());
	m_l2p1 = dynamic_cast<MESI_LLS_cache*>(unit1->get_lls());
	m_muxId1 = unit1->get_mux_cid();

	m_l1Id2 = unit2->get_llp_cid();
	m_l1p2 = dynamic_cast<MESI_LLP_cache*>(unit2->get_llp());
	m_l2p2 = dynamic_cast<MESI_LLS_cache*>(unit2->get_lls());
	m_muxId2 = unit2->get_mux_cid();

	m_l1Id3 = unit3->get_llp_cid();
	m_l1p3 = dynamic_cast<MESI_LLP_cache*>(unit3->get_llp());
	m_l2p3 = dynamic_cast<MESI_LLS_cache*>(unit3->get_lls());
	m_muxId3 = unit3->get_mux_cid();
	...
	Manifold::Connect(m_procId1, MockProc::OUT, &MockProc::handle_incoming,
	                  m_l1Id1, MESI_LLP_cache::PORT_PROC, &MESI_LLP_cache::handle_processor_request<Proc_req>,
			  MasterClock, MasterClock, 1, 1);
	Manifold::Connect(m_procId2, MockProc::OUT, &MockProc::handle_incoming,
	                  m_l1Id2, MESI_LLP_cache::PORT_PROC, &MESI_LLP_cache::handle_processor_request<Proc_req>,
			  MasterClock, MasterClock, 1, 1);
	Manifold::Connect(m_procId3, MockProc::OUT, &MockProc::handle_incoming,
	                  m_l1Id3, MESI_LLP_cache::PORT_PROC, &MESI_LLP_cache::handle_processor_request<Proc_req>,
			  MasterClock, MasterClock, 1, 1);


        //connect the components to the network
	//1st node
	const int NET_NODE_1_PORT = 0;
	Manifold::Connect(m_muxId1, MuxDemux::PORT_NET, &MuxDemux::handle_net<Mem_msg>,
	                  m_netId, NET_NODE_1_PORT, &MockNet::handle_incoming,
			  MasterClock, MasterClock, 1, 1);
	//2nd node
	const int NET_NODE_2_PORT = 1;
	Manifold::Connect(m_muxId2, MuxDemux::PORT_NET, &MuxDemux::handle_net<Mem_msg>,
	                  m_netId, NET_NODE_2_PORT, &MockNet::handle_incoming,
			  MasterClock, MasterClock, 1, 1);
	//3rd node
	const int NET_NODE_3_PORT = 2;
	Manifold::Connect(m_muxId3, MuxDemux::PORT_NET, &MuxDemux::handle_net<Mem_msg>,
	                  m_netId, NET_NODE_3_PORT, &MockNet::handle_incoming,
			  MasterClock, MasterClock, 1, 1);
	//MC
	const int NET_MC1_PORT = 3;
	Manifold::Connect(m_mcId1, MockMc::OUT, &MockMc::handle_incoming,
	                  m_netId, NET_MC1_PORT, &MockNet::handle_incoming,
			  MasterClock, MasterClock, 1, 1);
	//MC
	const int NET_MC2_PORT = 4;
	Manifold::Connect(m_mcId2, MockMc::OUT, &MockMc::handle_incoming,
        ...
        m_netp->add_node_id_to_port_map(NODEID_NODE_1, NET_NODE_1_PORT);
        m_netp->add_node_id_to_port_map(NODEID_NODE_2, NET_NODE_2_PORT);
        m_netp->add_node_id_to_port_map(NODEID_NODE_3, NET_NODE_3_PORT);
    }
	...
	Proc_req* req = new Proc_req(REQ_ID, NODEID_NODE_1, ADDR, Proc_req::LOAD);
	...
	MESI_LLS_cache* l2;
	int l2_node = m_l2map->lookup(ADDR);

	if(l2_node == NODEID_NODE_1)
	    l2 = m_l2p1;
	else if(l2_node == NODEID_NODE_2)
	    l2 = m_l2p2;
	else if(l2_node == NODEID_NODE_3)
	    l2 = m_l2p3;
	else
	    assert(0);
	...
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, manager->owner);
	...
	if(l2 == m_l2p1)
	    CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, creq.src_id);
	else if(l2 == m_l2p2)
	    CPPUNIT_ASSERT_EQUAL(NODEID_NODE_2, creq.src_id);
	else
	    CPPUNIT_ASSERT_EQUAL(NODEID_NODE_3, creq.src_id);
	...
	Proc_req* req2 = new Proc_req(REQ_ID2, NODEID_NODE_2, ADDR, Proc_req::LOAD);
	...
	CPPUNIT_ASSERT_EQUAL(true, manager->sharersList.get(NODEID_NODE_1));
	CPPUNIT_ASSERT_EQUAL(true, manager->sharersList.get(NODEID_NODE_2));
	...
	Proc_req* req = new Proc_req(REQ_ID, NODEID_NODE_1, ADDR, Proc_req::STORE);
	...
	Proc_req* req3 = new Proc_req(REQ_ID3, NODEID_NODE_3, ADDR, Proc_req::LOAD);
	...
	CPPUNIT_ASSERT_EQUAL(true, manager->sharersList.get(NODEID_NODE_1));
	CPPUNIT_ASSERT_EQUAL(true, manager->sharersList.get(NODEID_NODE_2));
	CPPUNIT_ASSERT_EQUAL(true, manager->sharersList.get(NODEID_NODE_3));
	...
	//schedule for the proc1 to send cache request
	const int REQ_ID2 = random();
	Proc_req* req2 = new Proc_req(REQ_ID2, NODEID_NODE_1, ADDR, Proc_req::LOAD);
	...
	//schedule for the proc1 to send cache request
	const int REQ_ID3 = random();
	Proc_req* req3 = new Proc_req(REQ_ID3, NODEID_NODE_1, ADDR, Proc_req::LOAD);
	...
	//schedule for the proc2 to send cache request
	const int REQ_ID2 = random();
	Proc_req* req2 = new Proc_req(REQ_ID2, NODEID_NODE_2, ADDR, Proc_req::STORE);
	Manifold::Schedule(When, &MockProc::send_req, m_procp2, req2);
	When += 50;
	...
	MESI_manager* manager = dynamic_cast<MESI_manager*>(l2->managers[l2->my_table->get_entry(ADDR)->get_idx()]);
	CPPUNIT_ASSERT_EQUAL(MESI_MNG_E, manager->state);
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_2, manager->owner);
	CPPUNIT_ASSERT_EQUAL(false, manager->sharersList.get(NODEID_NODE_1));
	CPPUNIT_ASSERT_EQUAL(false, manager->sharersList.get(NODEID_NODE_2));
	...
	//schedule for the proc3 to send cache request
	const int REQ_ID3 = random();
	Proc_req* req3 = new Proc_req(REQ_ID3, NODEID_NODE_3, ADDR, Proc_req::STORE);
	Manifold::Schedule(When, &MockProc::send_req, m_procp3, req3);
	When += 50;
	...
	MESI_manager* manager = dynamic_cast<MESI_manager*>(l2->managers[l2->my_table->get_entry(ADDR)->get_idx()]);
	CPPUNIT_ASSERT_EQUAL(MESI_MNG_E, manager->state);
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_3, manager->owner);
	CPPUNIT_ASSERT_EQUAL(0, manager->sharersList.count());
	...
	//schedule for the proc1 to send cache request
	const int REQ_ID3 = random();
	Proc_req* req3 = new Proc_req(REQ_ID3, NODEID_NODE_1, ADDR, Proc_req::STORE);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req3);
	When += 50;
	...
	for(int i=0; i<NUM_TAGS; i++) {
	    int REQ_ID = random();
	    Proc_req* req = new Proc_req(REQ_ID, NODEID_NODE_1, addrs[i], Proc_req::LOAD);
	    Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);
	}
	//set is full
	Ticks_t scheduledAt = When + MasterClock.NowTicks();
	...
	//schedule 1 more
	Proc_req* req = new Proc_req(random(), NODEID_NODE_1, ADDR, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);
	...
	for(int i=0; i<NUM_TAGS; i++) {
	    int REQ_ID = random();
	    Proc_req* req = new Proc_req(REQ_ID, NODEID_NODE_1, addrs[i], Proc_req::STORE);
	    Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);
	}
	//set is full
	Ticks_t scheduledAt = When + MasterClock.NowTicks();
	...
	{
	    Proc_req* req = new Proc_req(random(), NODEID_NODE_2, VICTIM_ADDR, Proc_req::LOAD);
	...
	//manager still has the VICTIM_ADDR line in S state.
	{
	    ...
	    int l2_node = m_l2map->lookup(VICTIM_ADDR);
	    ...
	    CPPUNIT_ASSERT_EQUAL(true, managerv->sharersList.get(NODEID_NODE_1));
	    CPPUNIT_ASSERT_EQUAL(true, managerv->sharersList.get(NODEID_NODE_2));
	}
	...
	{
	    Proc_req* req = new Proc_req(random(), NODEID_NODE_2, VICTIM_ADDR, Proc_req::STORE);
	    Manifold::Schedule(When, &MockProc::send_req, m_procp2, req);
	}
	When += 50;
	...
	//manager has the VICTIM_ADDR line in E state.
	{
	    ...
	    CPPUNIT_ASSERT_EQUAL(NODEID_NODE_2, managerv->owner);
	...
	assert(LLP_MSHR_SIZE == m_l1p1->mshr->assoc);
	...
	//schedule 1 more, which would stall
	Proc_req* req = new Proc_req(random(), NODEID_NODE_1, ADDR, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);
...
//cout << "addrs = " << hex<< addrs[i] <<dec<< "\n";
	...
	When += 7; //the following must be done before FWD_E reaches L1_1.
	//schedule a read request for L1_1 that causes eviction.
	Proc_req* req2 = new Proc_req(random(), NODEID_NODE_1, ADDR, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req2);
	...
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, manager->owner);
        ...
        //verify state of L2 cache
	l2_node = m_l2map->lookup(VICTIM_ADDR);
...
cout << "NUM_TAGS= " << NUM_TAGS << endl;
	...
	When += 5; //the following must be done before FWD_S reaches L1_1.
	           //if test case fails, check this first!!!
...
//cout << "ADDR = " << hex<< ADDR <<dec<< "\n";
	...
	for(int i=0; i<NUM_TAGS; i++) {
	    int REQ_ID = random();
//cout << "addrs = " << hex<< addrs[i] <<dec<< "\n";
	    Proc_req* req = new Proc_req(REQ_ID, NODEID_NODE_1, addrs[i], Proc_req::STORE); //Store so client is in M state.
	    Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);
	}
	//set is full
	When += 50;
	...
	for(int i=0; i<NUM_TAGS; i++) {
	    int REQ_ID = random();
	    Proc_req* req = new Proc_req(REQ_ID, NODEID_NODE_1, addrs[i], Proc_req::STORE); //store so client enters M state.
	    Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);
	}
	//set is full
	When += 50;
	...
	//Assuming there are 3 L2 nodes.
	CPPUNIT_ASSERT_EQUAL(3, (int)(dynamic_cast<MyLLSMap*>(m_l1p1->l2_map))->get_nodes());

	//ADDR1 sent to L1_1 causing L1_1 eviction; ADDR2 sent to L1_2 causing L2_1 eviction.
	//Therefore, they must both map to L2_1.

	//We have 3 L2 nodes: address *00, *01, *10 would map to the 3 nodes, *11 would map to
	//1st L2 node.
	const paddr_t PAGE_MASK = ~(0x3 << PAGE_BITS);


	//first create a random address.
	paddr_t ADDR2 = random(); //this is the one we use to cause L2 eviction.
	ADDR2 &= PAGE_MASK;
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, m_l1p1->l2_map->lookup(ADDR2));
	...
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, m_l1p1->l2_map->lookup(VICTIM_ADDR));
...
//cout << "l2 addrs = " << hex<< l2_addrs[i] <<dec<< "\n";
	...
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, m_l1p1->l2_map->lookup(ADDR1));
...
//cout << "l1 addrs = " << hex<< l1_addrs[i] <<dec<< "\n";
	...
	Proc_req* req = new Proc_req(random(), NODEID_NODE_2, ADDR2, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp2, req);

	When += 5; //the following must be done before DEMAND_I reaches L1_1.
	//schedule a read request for L1_1 that causes eviction.
	Proc_req* req2 = new Proc_req(random(), NODEID_NODE_1, ADDR1, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req2);
	...
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_2, manager2->owner);
...
//cout << "l1 addrs = " << hex<< l1_addrs[i] <<dec<< "\n";
    ...
    //!
    void test_request_race_l1_S_to_E_l2_DEMAND_I_0()
    {
	...
	paddr_t ADDR2 = random(); //this is the one we use to cause L2 eviction.
	ADDR2 &= PAGE_MASK;
	CPPUNIT_ASSERT_EQUAL(NODEID_NODE_1, m_l1p2->l2_map->lookup(ADDR2));
...
//cout << "l2 addrs = " << hex<< l2_addrs[i] <<dec<< "\n";
	...
	manager->sharersList.set(NODEID_NODE_1);
	manager->sharersList.set(NODEID_NODE_2);
	...
	When += 7; //the following request must be scheduled such that the above req gets to the L2 first.
	//schedule a WRITE request for L1_1 that causes S to E transition.
	Proc_req* req2 = new Proc_req(random(), NODEID_NODE_1, VICTIM_ADDR, Proc_req::STORE);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req2);
	...
	paddr_t ADDR = random();
        ...
        //fill up a set in L1_1
	const paddr_t L1_TAG_MASK = m_l1p1->my_table->get_tag_mask();
	const int L1_NUM_TAGS = m_l1p1->my_table->assoc;

	//create L1_NUM_TAGS unique tags in the same set as ADDR.
	set<paddr_t> addrset; //use a set to generate unique values

	paddr_t l1_addrs[L1_NUM_TAGS+1];

	for(int i=0; i<L1_NUM_TAGS+1; i++) {
	    paddr_t ad;
	    pair<set<paddr_t>::iterator,bool> ret; //return value; 2nd of the pair is false if the
						       //value is already in the set
	    //make sure ad is new and different from ADDR
	    do {
		ad = ADDR & (~L1_TAG_MASK); //clear the tag portion
		ad |= (random() & L1_TAG_MASK); // OR-in a random tag
		ad &= PAGE_MASK;
		if(ad != ADDR)
		    ret = addrset.insert(ad);
	...
	const paddr_t VICTIM_ADDR = l1_addrs[0];
	...
	//put VICTIM_ADDR into L2 hash table.
	m_l2p1->my_table->reserve_block_for(VICTIM_ADDR);
	hash_entry* entry = m_l2p1->my_table->get_entry(VICTIM_ADDR);
	entry->set_have_data(true);


	//put manager of VICTIM_ADDR in S state.
	entry = m_l2p1->my_table->get_entry(VICTIM_ADDR);
	MESI_manager* manager = dynamic_cast<MESI_manager*>(m_l2p1->managers[entry->get_idx()]);
	manager->state = MESI_MNG_S;
	manager->sharersList.set(NODEID_NODE_1);
	manager->sharersList.set(NODEID_NODE_2);

	//put l1_addrs[1] into L2 hash table, as it will also be evicted.
	m_l2p1->my_table->reserve_block_for(l1_addrs[1]);
	entry = m_l2p1->my_table->get_entry(l1_addrs[1]);
	entry->set_have_data(true);
	//set the manager's state to E
	manager = dynamic_cast<MESI_manager*>(m_l2p1->managers[entry->get_idx()]);
	manager->state = MESI_MNG_E;
	manager->owner = NODEID_NODE_1;
	...
	//put VICTIM_ADDR in L1_2
	m_l1p2->my_table->reserve_block_for(VICTIM_ADDR);
	hash_entry* entry2 = m_l1p2->my_table->get_entry(VICTIM_ADDR);
	//put client in S state.
	MESI_client* client2 = dynamic_cast<MESI_client*>(m_l1p2->clients[entry2->get_idx()]);
	client2->state = MESI_C_S;
	...
	Manifold::unhalt();
	Ticks_t When = 1;
	//schedule request for L1_1: this would cause silent eviction.
	Proc_req* req = new Proc_req(random(), NODEID_NODE_1, ADDR1, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req);

	When += 50;
	//schedule request for L1_2 for l1_addrs[1], so it enters S state, and later when it's
	//evicted on L1_1, the eviction can finish instantly.
	Proc_req* req1= new Proc_req(random(), NODEID_NODE_2, l1_addrs[1], Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp2, req1);


	When += 50;
	//schedule a WRITE request for L1_2 that causes DEMAND_I to be sent to L1_1.
	Proc_req* req2 = new Proc_req(random(), NODEID_NODE_2, VICTIM_ADDR, Proc_req::STORE);
	Manifold::Schedule(When, &MockProc::send_req, m_procp2, req2);

	When += 7; //the following request must be scheduled such that the above req gets to the L2 first.
	Proc_req* req3 = new Proc_req(random(), NODEID_NODE_1, VICTIM_ADDR, Proc_req::LOAD);
	Manifold::Schedule(When, &MockProc::send_req, m_procp1, req3);
	...
	CPPUNIT_ASSERT_EQUAL(0, (int)m_l1p2->stalled_client_req_buffer.size());
	//client in S state
	MESI_client* client1 = dynamic_cast<MESI_client*>(m_l1p1->clients[m_l1p1->my_table->get_entry(VICTIM_ADDR)->get_idx()]);
	CPPUNIT_ASSERT_EQUAL(MESI_C_S, client1->state);
	client2 = dynamic_cast<MESI_client*>(m_l1p2->clients[m_l1p2->my_table->get_entry(VICTIM_ADDR)->get_idx()]);
	CPPUNIT_ASSERT_EQUAL(MESI_C_S, client2->state);
	...
	CPPUNIT_ASSERT_EQUAL(2, (int)cresps.size());
	CPPUNIT_ASSERT_EQUAL(ADDR1, cresps[0].addr);
	CPPUNIT_ASSERT_EQUAL(VICTIM_ADDR, cresps[1].addr);


        //verify proc2
        vector<Proc_req>& cresps2 = m_procp2->get_cache_resps();
	CPPUNIT_ASSERT_EQUAL(2, (int)cresps2.size());
	CPPUNIT_ASSERT_EQUAL(VICTIM_ADDR, cresps2[1].addr);


        //verify mc
        vector<Mem_msg>& creqs1 = m_mcp1->get_reqs();
        vector<Mem_msg>& creqs2 = m_mcp2->get_reqs();
	//one request for ADDR1.
	CPPUNIT_ASSERT_EQUAL(1, (int)(creqs1.size() + creqs2.size()));
    }
/*
*/
    ...
    {
	CppUnit::TestSuite* mySuite = new CppUnit::TestSuite("MESI_LLP_LLS_cacheTest");
	/*
	*/
	//load miss
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_I_l2_I_0", &MESI_LLP_LLS_cacheTest::test_load_l1_I_l2_I_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_I_l2_E_0", &MESI_LLP_LLS_cacheTest::test_load_l1_I_l2_E_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_I_owner_M_0", &MESI_LLP_LLS_cacheTest::test_load_l1_I_owner_M_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_I_l2_S_0", &MESI_LLP_LLS_cacheTest::test_load_l1_I_l2_S_0));
	//load hit
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_E_l2_E_0", &MESI_LLP_LLS_cacheTest::test_load_l1_E_l2_E_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_S_l2_S_0", &MESI_LLP_LLS_cacheTest::test_load_l1_S_l2_S_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_load_l1_M_l2_E_0", &MESI_LLP_LLS_cacheTest::test_load_l1_M_l2_E_0));
	//store miss
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_I_l2_I_0", &MESI_LLP_LLS_cacheTest::test_store_l1_I_l2_I_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_I_l2_E_0", &MESI_LLP_LLS_cacheTest::test_store_l1_I_l2_E_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_I_l2_S_0", &MESI_LLP_LLS_cacheTest::test_store_l1_I_l2_S_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_I_owner_M_0", &MESI_LLP_LLS_cacheTest::test_store_l1_I_owner_M_0));
	//store hit
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_E_l2_E_0", &MESI_LLP_LLS_cacheTest::test_store_l1_E_l2_E_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_S_l2_S_0", &MESI_LLP_LLS_cacheTest::test_store_l1_S_l2_S_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_store_l1_M_l2_E_0", &MESI_LLP_LLS_cacheTest::test_store_l1_M_l2_E_0));
	//L1 eviction
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_l1_E_eviction_0", &MESI_LLP_LLS_cacheTest::test_l1_E_eviction_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_l1_M_eviction_0", &MESI_LLP_LLS_cacheTest::test_l1_M_eviction_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_l1_S_eviction_0", &MESI_LLP_LLS_cacheTest::test_l1_S_eviction_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_l1_S_eviction_1", &MESI_LLP_LLS_cacheTest::test_l1_S_eviction_1));
	//MSHR_STALL in L1
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_MSHR_STALL_in_l1_0", &MESI_LLP_LLS_cacheTest::test_MSHR_STALL_in_l1_0));
	//Request race
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_E_to_I_l2_FWD_E_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_E_to_I_l2_FWD_E_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_E_to_I_l2_FWD_S_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_E_to_I_l2_FWD_S_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_M_to_I_l2_FWD_E_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_M_to_I_l2_FWD_E_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_M_to_I_l2_FWD_S_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_M_to_I_l2_FWD_S_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_E_to_I_l2_DEMAND_I_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_E_to_I_l2_DEMAND_I_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_M_to_I_l2_DEMAND_I_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_M_to_I_l2_DEMAND_I_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_S_to_E_l2_DEMAND_I_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_S_to_E_l2_DEMAND_I_0));
	mySuite->addTest(new CppUnit::TestCaller<MESI_LLP_LLS_cacheTest>("test_request_race_l1_I_to_E_l2_DEMAND_I_0", &MESI_LLP_LLS_cacheTest::test_request_race_l1_I_to_E_l2_DEMAND_I_0));
	/*
	*/
...
Clock MESI_LLP_LLS_cacheTest :: MasterClock(MESI_LLP_LLS_cacheTest :: MASTER_CLOCK_HZ);

int main()
{
    Manifold :: Init();
    CppUnit::TextUi::TestRunner runner;
    runner.addTest( MESI_LLP_LLS_cacheTest::suite() );
    if(runner.run("", false))

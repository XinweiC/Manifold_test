...
public:
	...
	//SimpleRouter uses FCFS subcomponents.
	delete router->vca;
	delete router->swa;
	router->vca = new RRVcAllocator(router, router_params.no_ports, router_params.no_vcs);
	router->swa = new RRSwitchArbiter(router_params.no_ports, router_params.no_vcs);
	...
	hflit1->mclass = (random() % 2 == 0) ? PROC_REQ : MC_RESP;
    ...
    //! Create a SimpleRouter in a 4-node ring network. The router's id is 1. 
    //! Enter a head flit from both its interface and the West port, destined to
    //! East. Verify both vc states are FULL; call do_vc_allocation, both vc
    //! states change to VCA_REQUESTED; call do_vc_allocation again, only one changes
    //! to SWA_REQUESTED, since they both require the same output vc.
	...
	//only one advances to SWA_REQUESTED
    ...
    //! Create a SimpleRouter in a 4-node ring network. The router's id is 1. 
    //! Enter a head flit from both its interface and the West port, destined to
    //! East. Set all output vcs to busy. Verify both vc states are FULL;
    //! call do_vc_allocation, both vc states change to VCA_REQUESTED;
    //! call do_vc_allocation again, no one changes to SWA_REQUESTED.
	...
	//manually mark all VCs on the output port except FREE_VC as taken.
	...
	//no one would advance to SWA_REQUESTED
	CPPUNIT_ASSERT_EQUAL(VCA_REQUESTED, router->input_buffer_state[SimpleRouter::PORT_NI*VCS + VC0].pipe_stage);
	CPPUNIT_ASSERT_EQUAL(VCA_REQUESTED, router->input_buffer_state[SimpleRouter::PORT_WEST*VCS + VC1].pipe_stage);
    ...
    //! Enter a head flit from both its interface and the West port, destined to
    //! East. Verify both vc states are FULL; call do_vc_allocation, both vc
    //! states change to VCA_REQUESTED; call do_vc_allocation again, one changes
    //! to SWA_REQUESTED; now call do_switch_allocation(), the one in SWA_REQUESTED advances
    //! to SW_TRAVERSAL.
	...
	//one would advance to SWA_REQUESTED
	CPPUNIT_ASSERT_EQUAL(SWA_REQUESTED, router->input_buffer_state[SimpleRouter::PORT_NI*VCS + VC0].pipe_stage);
	CPPUNIT_ASSERT_EQUAL(VCA_REQUESTED, router->input_buffer_state[SimpleRouter::PORT_WEST*VCS + VC1].pipe_stage);
    ...
    //! destinations. Verify both vc states are FULL; call do_vc_allocation, both vc
    //! states change to VCA_REQUESTED; call do_vc_allocation again, both states change

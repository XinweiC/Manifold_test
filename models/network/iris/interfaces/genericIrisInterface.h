...
std::cerr << "@" << dec << manifold::kernel::Manifold::NowTicks() << " Interface " << id << " got flit from router: " << data->f->toString() << std::endl;
#endif
			...
			stat_sfpackets_in_from_router++;
                ...
                if ( data->f->type == HEAD ) {
                  ...
                  //std::cerr << "Interface " << std::dec << id << " @ " << std::dec << manifold::kernel::Manifold::NowTicks() << " addr: " << std::hex << ((HeadFlit*)data->f)->addr << " src: " << ((HeadFlit*)data->f)->src_id << " dst: " << ((HeadFlit*)data->f)->dst_id << std::endl;
...
#ifdef DEBUG_IRIS_INTERFACE
std::cerr << "@" << dec << manifold::kernel::Manifold::NowTicks() << " Interface " << id << " SEND Head/Body CREDIT vc " << ld->vc << std::endl;
#endif
...
//cerr << "@" << dec << manifold::kernel::Manifold::NowTicks() << " Receive IFID: " << dec << id  << "|| pkt type: " << data->get_type() << ", dst: " << data->get_dst() << " port: " << data->get_dst_port() << ", src: " << data->get_src() << " port: " << data->get_src_port() << " (Addr: " << hex << ((manifold::mcp_cache_namespace::Mem_msg*)(data->data))->get_addr()  << ")" << endl;
...
#ifdef DEBUG_IRIS_INTERFACE
std::cerr << "@" << dec << manifold::kernel::Manifold::NowTicks() << " Interface " << id << " SEND FLIT to router on VC " << winner << " Flit is " << f->toString();
if (f->type == HEAD)
  cerr << "| HEAD";
else if (f->type == BODY)
  cerr << "| BODY";
  ...
  cerr << " addr: " << std::hex << f->addr << " src: " << std::dec << f->src_id << " dst: " << std::dec << f->dst_id;
cerr << std::endl;
...
#ifdef DEBUG_IRIS_INTERFACE
std::cerr << "@" << dec << manifold::kernel::Manifold::NowTicks() << " Interface " << id << " SEND CREDIT to router on vc " << ld->vc << " after delivering packet to terminal." << std::endl;
#endif
...
GenNetworkInterface<T> :: send_credit_to_terminal(T* credit)
{
    ...
    Send(TERMINAL_PORT, credit);
//cerr << "SSSSS @ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " sends credit.\n";
//cerr.flush();
}
...
GenNetworkInterface<T>::to_flit_level_packet(FlitLevelPacket* flp, T* pkt, manifold::kernel::Ticks_t enter_time)
{
    ...
    //cerr << "*@" << dec << manifold::kernel::Manifold::NowTicks() << " pkt type: " << pkt->get_type() << ", dst: " << pkt->get_dst() << " port: " << pkt->get_dst_port() << ", src: " << pkt->get_src() << " port: " << pkt->get_src_port() << ", src id: " << hf->src_id << ", dst id: " << hf->dst_id << " (Addr: " << hex << ((manifold::mcp_cache_namespace::Mem_msg*)(pkt->data))->get_addr()  << ")";

    if (pkt->get_dst_port() == 234 || pkt->get_dst_port() == 567) { //LLP::LLP_ID, LLP::LLS_ID
        hf->term = CACHE;
        //cerr << " CACHE" << std::endl;
    } else if (pkt->get_type() == 456) { //MEM
        hf->term = MEMORY;
        //cerr << " MEMORY" << std::endl;
    } else {
        cerr << "Invalid terminal distination" << endl;
        exit(1);
    }

    hf->addr =  ((manifold::mcp_cache_namespace::Mem_msg*)(pkt->data))->get_addr();
...
//cerr << "@" << dec << manifold::kernel::Manifold::NowTicks() << " Send IFID: " << dec << id  << "|| pkt type: " << pkt->get_type() << ", dst: " << pkt->get_dst() << " port: " << pkt->get_dst_port() << ", src: " << pkt->get_src() << " port: " << pkt->get_src_port() << " (Addr: " << hex << ((manifold::mcp_cache_namespace::Mem_msg*)(pkt->data))->get_addr()  << ")" << endl;


        Send(TERMINAL_PORT, pkt);
//cerr << "SSSSS @ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " sends packet.\n";
//cerr.flush();
	#ifdef STATS
	stat_packets_out_to_terminal++;
	#endif
...
//cerr << "NI " << get_id() << " input notify " << when << "  is credit\n";
	return;
    }
    else {
//cerr << "NI " << get_id() << " input notify " << when << "  is msgggggggg\n";
...
//stat_pred_called++;
    ...
    manifold::kernel::Ticks_t now = manifold::kernel::Manifold::NowTicks();


    if(m_send_credit_tick.size() > 0 && m_send_credit_tick.front() == now) {
        //there is a credit scheduled for the current tick
//cerr << "@ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " no pred - credit scheduled\n";
...
//cerr << "@ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " no pred - output buffer\n";
	bp->update_output_tick(now);
        return;
    #if 0
        if(upstream_credits > 0) {
//cerr << "@ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " no pred - out buffer has packet, up_credits= " << upstream_credits << "\n";
	    return;
	}
	...
	while(m_recv_credit_tick.size() > 0 && m_recv_credit_tick.front() <= manifold::kernel::Manifold::NowTicks()) {
        ...
        if(credit_scheduled) {
//cerr << "@ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " no pred - out buffer has packet, credit scheduled\n";
	    return;
	}
#endif
    }
    ...
    //if ( found && (output_pkt_buffer.size() < UPSTREAM_BUFFER_SIZE) ) {
    if (found) {
//cerr << "@ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " no pred - whole packet\n";
	bp->update_output_tick(now);
        return;
    }
...
//cerr << "NI " << get_id() << " proc_in_buffer " << i << " not empty, len-size= " << ticks[i] << "\n";
		}
...
//cerr << "NI " << get_id() << " proc_in_buffer empty, router_in_buf " << i << " not empty, pktlen= " << ticks[i] << "\n";
		    }
			...
			ticks[i] = tick_from_router + 2; //next tick it will be in proc_in_buffer; the next after
			                                 //that for delivery
//cerr << "NI " << get_id() << " proc_in_buffer empty, router_in_buf empty " << i << " pred= " << ticks[i] << "\n";
		    }
		}
	    }
	...
	when += now;
//cerr << "@ " << manifold::kernel::Manifold::NowTicks() << " NI " << get_id() << " do pred, tick= " << when << "\n";

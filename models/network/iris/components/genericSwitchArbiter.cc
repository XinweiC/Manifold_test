...
{
#ifdef IRIS_DBG
std::cerr << "SWA request: op= " << oport << " ov= " << ovc << " ip= " <<inport << " iv= " <<ivc <<std::endl;
#endif
...
#ifdef IRIS_DBG
std::cerr << "Router " << router_id << " Switch Allocation, oport= " << oport << " requested port-vc: ";
for(int i=0; i<ports*vcs; i++) {
if(requested[oport][i])
std::cerr << " " << i/vcs << "-" << i%vcs;
}
std::cerr << std::endl;
#endif

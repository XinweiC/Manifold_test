...
std::cerr << "Interface " << id << " got flit from router: " << data->f->toString() << std::endl;
#endif
                if ( data->f->type == TAIL )
...
#ifdef DEBUG_IRIS_INTERFACE
                    std::cerr << " SEND Head/Body CREDIT vc " << ld->vc << std::endl;
#endif
...
#ifdef DEBUG_IRIS_INTERFACE
std::cerr << "Interface " << id << " SEND CREDIT to router after delivering packet to terminal." << std::endl;
#endif

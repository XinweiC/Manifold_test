...
GenericRC::route_ring_uni(HeadFlit* hf)
{
    ...
    cerr << "ERROR: dint return yet " << endl;
...
GenericRC::push (Flit* f, uint ch )
{
    ...
    uint myx = (int)(node_id%grid_size);
    uint destx = (int)(f->dst_id%grid_size);
    uint myy = (int)(node_id/grid_size);
    uint desty = (int)(f->dst_id/grid_size);

#if 0
    cerr << " @ " << std::dec << manifold::kernel::Manifold::NowTicks() << " myx: " << myx << " myy: " << myy << " dstx: " << destx << " dsty: " << desty;
    cerr << " | node_id " << node_id << " src_id " << f->src_id << " dst_id " << f->dst_id << " addr: " << std::hex << f->addr;
  ...
  //cerr << " || HEAD " << " out_port " << addresses[ch].out_port << " vcs " <<	addresses[ch].channel  << " valid: " << addresses [ch].route_valid << std::endl;  
    }
    else if(f->type == TAIL)
    {
  ...
  //cerr << " || TAIL " << " out_port " << addresses[ch].out_port << " vcs " <<	addresses[ch].channel  << " valid: " << addresses [ch].route_valid << std::endl;  
    ...
    //cerr << " || BODY " << " out_port " << addresses[ch].out_port << " vcs " <<	addresses[ch].channel  << " valid: " << addresses [ch].route_valid << std::endl;  
        if( !addresses[ch].route_valid)
        {

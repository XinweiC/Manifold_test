    ...
    node_lp(node_lp)
{
    ...
    {
        cerr<<"Bad node to lp mapping!!"<<endl;
	exit(1);
    }
    ...
    for ( uint i=0; i< node_lp->size(); i++)
    {
        ...
        //std::cerr<< "0.0 N " << i << " " << i << " " << 0 << std::endl;
#endif        
        router_ids.push_back( manifold::kernel::Component::Create<SimpleRouter>(node_lp->at(i), i, &i_p_rt) ); 
// 	cerr<<"node id: "<< node_lp->at(i).node_id <<" node lp: "<<node_lp->at(i).lp<<endl;
#ifdef IRIS_TEST
        if ((i%x_dim != 0) && (i%x_dim != (x_dim - 1)))
        {
           if (grid_count%2 == 0)
           {
              ...
              //std::cerr<< "0.0 N " << i  + no_nodes << " " << i%x_dim + 6<< " " << 1 + grid_count*2<< std::endl;
           }
           if (grid_count%2 == 1)
           {
              ...
              //std::cerr<< "0.0 N " << i  + no_nodes << " " << double(i%x_dim) + 6 + 0.5<< " " << 1 + grid_count*2<< std::endl;
           }           
        }
        else
        {
           if (grid_count%2 == 0)
           {
               ...
               //std::cerr<< "0.0 N " << i  + no_nodes << " " << i%x_dim + 6<< " " << 2 + grid_count*2 << std::endl;
           }  
           if (grid_count%2 == 1)
           {
               ...
               //std::cerr<< "0.0 N " << i  + no_nodes << " " << double(i%x_dim) + 6 + 0.5<< " " << 2 + grid_count*2 << std::endl;
           }  
        }
...
Torus6p<T>::connect_interface_routers()
{
	...
	manifold::kernel::Manifold::Connect(router_ids.at(i), SimpleRouter::PORT_MC, 
		...
		outFile_data<< "0.0 L " << interfaces.at(i)->id << " " << routers.at(i)->node_id + x_dim*y_dim<< std::endl;
 		//cerr<< "0.0 L " << interfaces.at(i)->id << " " << routers.at(i)->node_id + x_dim*y_dim<< std::endl;
    }
	#endif
    }
}
...
Torus6p<T>::connect_routers()
{
    ...
    for ( uint i=0; i<y_dim; i++) { //for all rows
		...
		//cerr<<"router: "<<i*x_dim + j<<" at lp: "<<node_lp->at(i*x_dim + j)<<" WEST to router: "<<i*x_dim + j-1<<" at lp: "<<node_lp->at(i*x_dim + j-1)<<" EAST "<<endl;
	    }  
    ...
    for ( uint i=1; i<y_dim; i++) { //starting from 2nd row
		...
		//cerr<<"router: "<<i*x_dim + j<<" at lp: "<<node_lp->at(i*x_dim + j)<<" NORTH to router: "<<(i-1)*x_dim + j<<" at lp: "<<node_lp->at((i-1)*x_dim + j)<<" SOUTH "<<endl;
	    }  
	...
	if (node_lp->at(i*x_dim) != node_lp->at(i*x_dim+x_dim-1)){
	    ...
	    //cerr<<"router: "<<i*x_dim<<" at lp: "<<node_lp->at(i*x_dim)<<" WEST to router: "<<i*x_dim+x_dim-1<<" at lp: "<<node_lp->at(i*x_dim+x_dim-1)<<" EAST "<<endl;
	}
	...
	if (node_lp->at(i) != node_lp->at((y_dim-1)*x_dim+i)){
	    ...
	    //cerr<<"router: "<<i<<" at lp: "<<node_lp->at(i)<<" NORTH to router: "<<(y_dim-1)*x_dim+i<<" at lp: "<<node_lp->at((y_dim-1)*x_dim+i)<<" SOUTH "<<endl;
	}

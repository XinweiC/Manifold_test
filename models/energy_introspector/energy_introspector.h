...
namespace EI {
  ...
  public:
    ...
    void compute_leakage(double time_tick, double period, string module_name);
    ...
    template <typename T>
    void update_variable_partition(string name, string updater, T value)
    {
      ...
      if(partition_it != partition.end())
      {
        ...
        for(vector<string>::iterator module_it = partition_it->second.module.begin(); module_it != partition_it->second.module.end(); module_it++) {
            //std::cerr <<name<<", "<<module_it->c_str()<<endl;
            update_variable(module_it->c_str(), updater, value);
        }
      }
      else
      {
        EI_WARNING("EI","cannot find pseudo partition "+name+" in update_energy()");
      }
    }

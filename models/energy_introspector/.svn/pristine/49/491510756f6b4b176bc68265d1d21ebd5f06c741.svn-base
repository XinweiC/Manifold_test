...
void intsim(intsim_chip_t *chip, intsim_param_t *param, char* output_file) {
	...
	if(!chip->gate_sizing(param)) {
		...
		fprintf(stderr,"ERROR: The specified clock frequency cannot be reached with this logic depth, supply voltage, and threshold voltage.\n");
		return;
	}
	...
	// Step 10: Data Output
	chip->output(param,intsim_total_power,output_file); 
}

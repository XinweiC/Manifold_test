...
intsim_param_t::intsim_param_t()
{
  memset(this,0,sizeof(intsim_param_t));
}

void intsim_param_t::setup(bool use_default) 
{
  if(use_default)
  {
    ...
    s = 3; // Design rule for vias - fixed
    ...
    kai = 4.0/(fo+3.0); // Point-to-point to net length correction factor
    alpha_wire = fo/(fo+1.0); // Alpha wire
    ...
    kp = 2; // Ratio of widths of power and signal wires - fixed
    kc = 4; // Ratio of widths of clock and signal wires - fixed
  ...
  else
  {
    if(f == 0.0)
    {
      fprintf(stderr,"clock frequency not defined\n");
      exit(1);
    }
    if(A == 0.0)
    {
      fprintf(stderr,"area not defined\n");
      exit(1);
    }
    if(ngates == 0.0)
    {
      fprintf(stderr,"number of gates not defined\n");
      exit(1);
    }
    ...
    switch((int)(F*1e9))
    {
      case 16:
      {
        Vdd = Vdd?Vdd:0.7; // vdd[0] from McPAT
        Vt = Vt?Vt:0.1489; // v_th[0] from McPAT
        tox = tox?tox:0.5e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.0); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:1.998; // horiz_dielectric_constant[1][0]
        L = 0.00315; // Lelec[0] from McPAT
        Idsat_spec = 2768.4e-6; // I_on_n[0] from McPAT
        alpha = alpha?alpha:1.38; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(0.7-0.1489)*Vdd/0.7*1e6;
        Ileak_scaling = 1e6;
      ...
      case 22:
      {
        Vdd = Vdd?Vdd:0.8; // vdd[0] from McPAT
        Vt = Vt?Vt:0.1395; // v_th[0] from McPAT
        tox = tox?tox:0.55e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.0); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:2.104; // horiz_dielectric_constant[1][0]
        L = 0.00468; // Lelec[0] from McPAT
        Idsat_spec = 2626.4e-6; // I_on_n[0] from McPAT
        alpha = alpha?alpha:1.38; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(0.8-0.1395)*Vdd/0.8*1e6;
        Ileak_scaling = 1e6;
      ...
      case 32:
      {
        Vdd = Vdd?Vdd:0.9; // vdd[0] from McPAT
        Vt = Vt?Vt:0.21835; // v_th[0] from McPAT
        tox = tox?tox:0.5e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.41); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:2.214; // horiz_dielectric_constant[1][0]
        L = 0.01013; // Lelec[0] from McPAT
        Idsat_spec = 2211.7e-6; // I_on_n[0] from McPAT
        alpha = alpha?alpha:1.38; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(0.9-0.21835)*Vdd/0.9*1e6;
        Ileak_scaling = 1e6;
      ...
      case 45:
      {
        Vdd = Vdd?Vdd:1.0; // vdd[0] from McPAT
        Vt = Vt?Vt:0.18035; // v_th[0] from McPAT
        tox = tox?tox:0.65e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.41); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:2.46; // horiz_dielectric_constant[1][0]
        L = 0.01345; // Lelec[0] from McPAT
        Idsat_spec = 2046.6e-6; // I_on_n[0] from McPAT
        alpha = alpha?alpha:1.38; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(1.0-0.18035)*Vdd/1.0*1e6;
        Ileak_scaling = 1e6;
      ...
      case 65:
      {
        Vdd = Vdd?Vdd:1.1; // vdd[0] from McPAT
        Vt = Vt?Vt:0.19491; // v_th[0] from McPAT
        tox = tox?tox:1.1e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.41); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:2.734; // horiz_dielectric_constant[1][0]
        L = 0.019; // Lelec[0] from McPAT
        Idsat_spec = 1197.2e-6; // I_on_n[0] from McPAT
        alpha = alpha?alpha:1.38; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(1.1-0.19491)*Vdd/1.1*1e6;
        Ileak_scaling = 1e6;
      ...
      case 90:
      {
        Vdd = Vdd?Vdd:1.2; // vdd[0] from McPAT
        Vt = Vt?Vt:0.23707; // v_th[0] from McPAT
        tox = tox?tox:1.2e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.45); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:3.038; // horiz_dielectric_constant[1][0]
        L = 0.0266; // Lelec[0] from McPAT
        Idsat_spec = 1076.9e-6; // I_on_n[0] from McPAT
        alpha = alpha?alpha:1.22; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(1.2-0.23707)*Vdd/1.2*1e6;
        Ileak_scaling = 1e6;
      ...
      case 180:
      {
        Vdd = Vdd?Vdd:1.5; // vdd[0] from McPAT
        Vt = Vt?Vt:0.4407; // v_th[0] from McPAT
        tox = tox?tox:2.4e-9; // t_ox[0] from McPAT
        drive_p_div_n = drive_p_div_n?drive_p_div_n:(1.0/2.45); // n_to_p_eff_curr_drv_ratio[0] from McPAT
        ar = ar?ar:2.0; // aspect_ratio[1][0] from McPAT
        er = er?er:3.038; // horiz_dielectric_constant[1][0]
        ...
        alpha = alpha?alpha:1.22; // gmp_to_gmn_multiplier[0] from McPAT
        Idsat_scaling = (Vdd-Vt)/(1.5-0.4407)*Vdd/1.5*1e6;
        Ileak_scaling = 1e6;
      ...
      default:
      {
        fprintf(stderr,"unknown feature size %dnm\n",(int)(F*1e9));
        exit(1);
      }
    }
    ncp = ncp?ncp:10; // Number of gates on a critical path
    k = k?k:4.0; // Rent's constant k
    p = p?p:0.6; // Rent's constant p
    a = a?a:0.1; // Activity factor
    Vdd_spec = Vdd_spec?Vdd_spec:Vdd; // Vdd at which Idsat and Ileak are specified
    Vt_spec = Vt_spec?Vt_spec:Vt; // Vt at which Idsat and Ileak are specified
    subvtslope_spec = subvtslope_spec?subvtslope_spec:(100e-3); // Subthreshold slope at 85 degrees Celsius
    F1 = F; // Minimum wire pitch
    s = 3; // Design rule for vias - fixed
    er = er?er:2.0; // Dielectric permitivity
    rho = rho?rho:1.95e-8; // Resistivity of copper
    ar = ar?ar:2.0; // Aspect ratio of wiring levels
    R_coeff = R_coeff?R_coeff:0.5; // Reflectivity coefficient at grain boundaries for copper
    p_size = p_size?p_size:0.5; // Specularity parameter
    npower_pads = npower_pads?npower_pads:600.0; // Number of power pads
    pad_to_pad_distance = pad_to_pad_distance?pad_to_pad_distance:300e-6; // Average distance from one power pad to the next
    pad_length = pad_length?pad_length:50e-6; // Length of a pad
    ir_drop_limit = ir_drop_limit?ir_drop_limit:0.02; // IR drop limit in percentage, half for each global and local
    router_eff = router_eff?router_eff:0.5; // Router efficiency
    rep_eff = rep_eff?rep_eff:0.5; // Repeaters efficiency
    fo = fo?fo:3.0; // Average fan-out of logic gates
    margin = margin?margin:0.2; // Percentage of clock cycle lost due to the process variation and clock skew
    D = D?D:3e-3; // Max span of H tree that needs to be driven
    latches_per_buffer = latches_per_buffer?latches_per_buffer:20.0; // Latches per buffer
    clock_factor = clock_factor?clock_factor:1.0; // Clock factor (number of latches = clock factor x total number of gates / number of gates on a critical path)
    clock_gating_factor = clock_gating_factor?clock_gating_factor:0.4; // Percentage of local clock power saved by clock gating
    kai = 4.0/(fo+3.0); // Point-to-point to net length correction factor
    alpha_wire = fo/(fo+1.0); // Alpha wire
    max_tier = max_tier?max_tier:40;
    
    // TODO: fix this
    Ileak_spec = I_off_n[(int)temp-300]*Ileak_scaling*F;
    Idsat_spec *= (Idsat_scaling*F);
    ro = calc_ro(Vdd,Vt,Vdd_spec,Vt_spec,Idsat_spec,alpha);
    co = calc_co(tox,F);
